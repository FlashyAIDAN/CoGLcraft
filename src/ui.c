...
struct UIrenderer MakeUIrenderer(struct Shader shader, unsigned int width, unsigned int height)
{
	...
	SetShaderMatrix4(&uiRenderer.shader, "projection", glms_ortho(-((float)(width / igcd(width, height)) / 2.0f / 10.0f), (float)(width / igcd(width, height)) / 2.0f / 10.0f, -((float)(height / igcd(width, height)) / 2.0f / 10.0f), (float)(height / igcd(width, height) / 2.0f / 10.0f), -1.0f, 1.0f), false);
    ...
    glBindVertexArray((int)uiRenderer.VAO);
...
void RenderSpriteUI(struct UIrenderer *uiRenderer, struct Texture2D *texture, vec2s position, float rotation, vec2s size, vec3s color)
{
	...
	SetShaderMatrix4(&uiRenderer->shader, "model", uiModelBlock, true);
